...
async function articleInFn(ctx) {
    return new Promise((resolve, reject) => {
        articleList.count().exec(function (err, data) {
            console.log(ctx.query.current != undefined ? ((ctx.query.current - 1) * 10) : 0)
            articleList.find({}, {}).sort({"updatedAt": -1}).limit(ctx.query.limit!=undefined?parseInt(ctx.query.limit):10).skip(ctx.query.current != undefined ? ((ctx.query.current - 1) * ctx.query.limit) : 0).exec(function (err, listdata) {
                resolve({status: 'ok', data: {total: data, listdata: listdata}})
            })
        })
    })
...
module.exports = {
    ...
    AboutUsInFn:AboutUsInFn,
    articleInFn:articleInFn
    // 'GET /': fn_index,
