...
class ProductClassify extends React.Component {
  ...
  //添加按钮
  addCate() {
    this.setState({submitLoading: false, visible: true, ModalTitle: '添加产品分类',imgVisible:false});
    this
      .props
      .form
      .resetFields();
    this
      ...
      this.getProductSeclect(this.state.onSelectLength[0])
  }
  //树形删除机构
  delNode() {
    //取消按钮
  ...
  //通过机构ID查询 网站列表
  getProductSeclect(id,evSelectId){
    ...
    fetch(serverSrc.__proto__.serverSrc + "admin/cate/query", opts).then((response) => {
      ...
      if (responseText.status == '200') {
        ...
        this.setState({productDataSource: responseText.data});
      } else {
      }
    }).catch((error) => {
      alert(error)
    ...
    this
      ...
      .validateFields((err, values) => {
        ...
        if (!err) {
          ...
          if (this.state.ModalTitle == '添加产品分类') {
            ...
            fetch("http://139.199.77.158/zjh/admin/cate/update", opts).then((response) => {
              //你可以在这个时候将Promise对象转换成json对象:response.json() 转换成json对象后return，给下一步的.then处理
              return response.json()
            }).then((responseText) => {
            ...
            // jsonp({   url: 'admin/cate/update',   key: 'callback',   data: {
            // token:_thisState.state.localToken,     title:values.title,
            // parentId:values.parentId,     image:values.image,
            // ordering:values.ordering,     shared:values.shared,     type:"product",
            // publish:values.publish,     siteId:_thisState.state.onSelectLength[0],
            // id:_thisState.state.setSiteId   },   callback: function(ret) {
            // _thisState.setState({submitLoading:false});     if(ret.status=='200'){
  ...
  renderTreeNodes(data) {
    return data.map((item) => {
      ...
      // key={item.orgId} dataRef={item}>
      // {this.renderTreeNodes(item.children)}         </TreeNode>     ); }
  ...
  editModal(ev) {
    ...
    this.setState({submitLoading: false,doneImgUrl:ev.image,visible: true,imgVisible:true,setSiteId: ev.id, ModalTitle: '编辑产品分类'});
  ...
  // handleUpload(e) {
    // console.log(e,444444444444444444) const { fileList } = this.state; const
    // formData = new FormData(); fileList.forEach((file) => {
    ...
    // false,   data: formData,   success: () => {     this.setState({
    // fileList: [],       uploading: false,     });     message.success('upload
    // successfully.');   },   error: () => {     this.setState({       uploading:
    // false,     });     message.error('upload failed.');   }, });
  // }
  ...
  upfileinput(e) {
    ...
    if( this.refs.upElem.refs.input.files[0]!=undefined){
      ...
      // this.refs.upElem.refs.input.files.FileList.forEach((file) => {   formData.append('image', file); });
      formData.append("token", this.state.localToken);
  ...
  UpBtn(){
    ...
    this.refs.upElem.refs.input.click()
  }
  //搜索方法
  onInputChange(e) {
  ...
  render() {
    ...
    const columns = [
      {
        ...
        render: (text, record) => {
          ...
          return  (
              <div>
                <Popconfirm title="确认删除该条产品分类?" onConfirm={() => this.onDelete(record.id)}>
                  <a href="#" style={{
                    marginRight: 10
                  }}>删除</a>
                </Popconfirm>
    ...
    // selectedRowKeys.length > 0; console.log(FootCss) 上传 页面渲染
    ...
    //   onPreview: (file) => {
    //     this.setState({
    //       previewImage: serverSrc.__proto__.serverSrc + file.response.data,
    //       previewVisible: true
    //     });
    //   }
    // };
                  ...
                  {getFieldDecorator('image', {
                    ...
                    <div>
                      <Input
                        ...
                        type="file"
                        placeholder=""/>
                        <b  className="ant-btn"  onClick={this.UpBtn.bind(this)} style={{lineHeight:2.15}}> 上传</b> 
                        <img  style={{marginLeft:20,height:32,verticalAlign:'top',display:this.state.imgVisible?'inline-block':'none'}} src={serverSrc.__proto__.serverSrc+this.state.doneImgUrl}  ></img>
                    </div>
                  )}
                </FormItem>
                <Modal
                ...
                <FormItem label="上级分类" {...formItemLayout }>
                  {getFieldDecorator('parentId', {
                    ...
                    <Select style={{
                      ...
                      {this.addTreeNodes(this.state.productDataSource)}
                    </Select>
...
ProductClassify = Form.create({})(ProductClassify);
